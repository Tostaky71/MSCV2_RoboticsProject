...
All this part has to control a robot automatiquely in it's environnment. the evironnement is composed os a 2D map which allow the robot to localise itself (create initial coordinate points) and a 3D map which will be created by the robot.
The 3D map is a colored point cloud mapping designed by the RGB-D depth camera Kinect by Microsoft. When the robot has it's 2D map, it will obtain and match the initial coordinates with the map and then move with the goal coordinates defined to finally create the 3D map of the area. On the 3D map obtained we can reconstruct a very realistic colored view and extract the details we want, for example if we want to reconstruct in 3D a big monument, we can extract the images of damages due to time from a drone and try to repair it by simulations before.
